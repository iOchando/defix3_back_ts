...
function swapToken(req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            if (!fromCoin || !toCoin || !fromDefix || !priceRoute || !privateKey || !blockchain || !code)
                return res.status(400).send();
            if (!(yield (0, _2fa_1.validation2FA)(fromDefix, code)))
                return res.status(401).send();
            let swapHash;
            if (blockchain === "ETH") {
                swapHash = yield (0, eth_services_1.swapTokenETH)(fromCoin, privateKey, priceRoute);
            }
            else if (blockchain === "BNB") {
                swapHash = yield (0, bsc_services_1.swapTokenBSC)(fromCoin, privateKey, priceRoute);
            }
            else {
                swapHash = false;
            }
            ...
            if (!swapHash)
                return res.status(500).send();
            const transaction = yield (0, utils_1.saveTransaction)(fromDefix, fromDefix, coin, blockchain, amountA, swapHash.address, swapHash.address, swapHash, 'DEX');
