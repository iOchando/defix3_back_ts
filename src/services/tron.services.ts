...
async function transactionTRON(fromAddress: string, privateKey: string, toAddress: string, coin: string, amount: number) {
  try {
    ...
    const tx = await tronWeb.transactionBuilder.sendTrx(toAddress, srcAmount)
...
async function transactionTokenTRON(fromAddress: string, privateKey: string, toAddress: string, amount: number, srcToken: any) {
  try {
    const balance = await getBalanceTokenTRON(fromAddress, srcToken.contract, srcToken.decimals);
    if (balance < amount) {
      console.log('Error: No tienes suficientes fondos para realizar la transferencia');
      return false;
    }
...
async function payCommissionTRON(fromAddress: string, privateKey: string, toAddress: string, amount: number) {
  try {
    tronWeb.setAddress(fromAddress)
...
export { createWalletTRON, isAddressTRON, getBalanceTRON, getBalanceTokenTRON, transactionTRON, transactionTokenTRON };